...
-- 2. Migra dados existentes de colunas WAHA para Config JSONB (idempotente)
DO $$
BEGIN
    -- Only migrate if old columns exist
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'channels' AND column_name = 'waha_base_url'
    ) THEN
        UPDATE channels
...
-- 4. Remove colunas específicas do WAHA (agora no Config JSONB)
...
-- 5. Adiciona índice no external_id para buscas rápidas
...
-- 6. Adiciona índice GIN no Config JSONB para queries eficientes
CREATE INDEX IF NOT EXISTS idx_channels_config_gin ON channels USING GIN (config);

-- 7. Adiciona comentários para documentação
